...
if (isset($_POST['submit'])) {
    ...
    if($_FILE["image"]["error"]==0){
        $query = "UPDATE news set news_img = '$file_name',news_name = '$name',news_nameAr = '$nameAr',news_desc = '$description',news_desc_Ar='$desc_Ar',news_date = '$date' where news_id = {$_GET['news_id']}";
        mysqli_query($conn, $query);
        header("location:manage_news.php");
    }
    else{
        $query = "UPDATE news set news_name = '$name',news_nameAr = '$nameAr',news_desc = '$description',news_desc_Ar='$desc_Ar',news_date = '$date' where news_id = {$_GET['news_id']}";
            mysqli_query($conn, $query);
            header("location:manage_news.php");        
    }
}
                            ...
                            <form action="" method="post" novalidate="novalidate" enctype="multipart/form-data">
                                ...
                                <div class="form-group">
                                    ...
                                    <input id="cc-number" name="nameAr" type="text" class="form-control cc-number identified visa" data-val="true"
                                           data-val-required="Please enter the card number" data-val-cc-number="Please enter a valid card number"
                                           autocomplete="cc-number" value="<?php echo $adminset['news_nameAr']; ?>">
                                    <span class="help-block" data-valmsg-for="cc-number" data-valmsg-replace="true"></span>
                                </div>
                                <div class="form-group">
